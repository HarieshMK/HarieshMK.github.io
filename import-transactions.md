...
function displayPreview() {
    document.getElementById('preview-body').innerHTML = parsedData.map(d => `
        <tr style="border-bottom: 1px solid #1e293b;"><td style="padding: 8px 0;">${d.date}</td><td>${d.share}</td><td style="text-align: right; color: ${d.amount > 0 ? '#4ade80' : '#f87171'};">₹${Math.abs(d.amount).toFixed(2)}</td></tr>
    `).join('');
...
function resetPage() {
    ...
    document.getElementById('btn-import-more').disabled = false;
    document.getElementById('btn-import-more').innerText = "Import & Add More";
}
